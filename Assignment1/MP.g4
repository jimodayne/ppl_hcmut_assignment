...
varDeclarationList
	:
		...
		| identifierList COL componentType arrDeclaration SEMI
...
identifierList
	: identifier (COM identifierList)*
	;

arrDeclaration
	: LSB expression DOD expression RSB OF primitiveType
	;
...
assignmentStatement
	: lhs ASSIGN assignmentStatement
...
indexExpression
	: identifier (LB expression? RB)? LSB expression RSB
	| LB expressionList? RB LSB expression RSB
...
ERROR_CHAR: . {raise ErrorToken(self.text)}; // error token - find exception in file lexererr.py
